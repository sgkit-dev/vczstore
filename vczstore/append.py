...
async def _append_async(vcz1, vcz2, *, show_progress=False, io_concurrency):
    root1, root2 = (
        await zarr_async.open_group(store=vcz1, mode="r+"),
    ...
    async for name, arr1 in root1.arrays():
        if name.startswith("call_"):
            if name not in root2_arrays:
                raise ValueError(
                    f"append requires {name!r} to be present in both stores"
                )
            arr2 = root2_arrays[name]
            _assert_append_arrays_compatible(name, arr1, arr2)
            call_arrays.append((name, arr1, arr2))
    ...
    _assert_variant_chunk_alignment(
        [(name, arr2) for name, _, arr2 in call_arrays],
        variant_chunk_size=variant_contig2.chunks[0],
        operation="append",
    )
